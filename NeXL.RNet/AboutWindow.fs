...
module AboutWindow =
    ...
    let create() =
        ...
        label1.Text <- "Excel Connector for R. Powered by NeXL Technology"
        ...
        link.RequestNavigate.Add(fun args ->         
                                    try
                                        System.Diagnostics.Process.Start("https://github.com/Statfactory/NeXL.RNet") |> ignore
                                    with
                                        | _ -> ())
        link.NavigateUri <- new Uri("https://github.com/Statfactory/NeXL.RNet")
